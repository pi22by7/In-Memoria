...
function runCommand(args: string[], cwd: string): void {
  ...
  if (process.env.CI === 'true') {
    console.log(`🔧 NAPI override (env): ${env.NAPI_RS_NATIVE_LIBRARY_PATH ?? 'unset'}`);
    console.log(`🔧 NAPI override (resolved): ${nativeLib ?? 'none found'}`);
    console.log(`🔧 IN_MEMORIA_DEBUG_PHP: ${env.IN_MEMORIA_DEBUG_PHP ?? 'unset'}`);
  }

  if (!env.NAPI_RS_NATIVE_LIBRARY_PATH) {
    console.warn('⚠️  No native binding override detected; falling back to default resolution');
  }
  ...
  for (const fixture of fixtures) {
    ...
    for (const root of fixture.roots) {
      ...
      if (process.env.CI === 'true') {
        ...
        console.log(`🔍 Fixture "${fixture.name}" (${root}) contains ${phpFiles.length} PHP files`);
        if (phpFiles.length > 0) {
          console.log(`   • ${phpFiles.slice(0, 10).join(', ')}${phpFiles.length > 10 ? '…' : ''}`);
        }
      }
